...
- `main.agent.md`: a chat agent that can use a Python dynamic session pool for code execution.
...
This sample currently points at a temporary `azurefunctions-agents-runtime` `.whl` URL in `src/requirements.txt`. The dependency will be updated when the package is published.
...
Email delivery is disabled by default. To deploy the sample without Office 365 Outlook, do not set `EMAIL_RECIPIENT`.

To enable email delivery, set `EMAIL_RECIPIENT` before running `azd up`:

```bash
azd env set EMAIL_RECIPIENT you@example.com
```
...
- A Microsoft Foundry account, project, and `gpt-5.4` deployment
- An Azure Container Apps dynamic session pool
- Optional Office 365 Outlook Connector Gateway resources, when `EMAIL_RECIPIENT` is set
...
After `azd up` completes, open the Function App endpoint printed by `azd`. The default chat UI is served from the app root.

The chat agent can use Python code execution through the dynamic session pool. It does not have access to the Office 365 Outlook email tool.
...
When `EMAIL_RECIPIENT` is blank, the timer agent does not try to send email. It returns the complete digest as its final response so the digest appears in logs.

When `EMAIL_RECIPIENT` is set and the Office 365 connection is authenticated, the timer agent sends the digest to that recipient using the Office 365 Outlook MCP tool.
